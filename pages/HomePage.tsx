...
import {
  ShieldCheck, Zap, Layers, Wrench, Cpu, GitMerge, PackageCheck, ArrowRight, Quote, ChevronDown
} from 'lucide-react';
...
const InfoCard: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
    <div className="bg-white dark:bg-slate-800 p-8 rounded-xl border border-border dark:border-slate-700 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 h-full text-center">
        <div className="text-accent w-16 h-16 mx-auto mb-6 flex items-center justify-center">
            {icon}
        </div>
        <h3 className="text-xl font-bold font-heading text-text-DEFAULT dark:text-slate-200 mb-2">{title}</h3>
...
const TestimonialCard: React.FC<{ testimonial: Testimonial }> = ({ testimonial }) => (
    <div className="bg-white dark:bg-slate-800 p-8 rounded-lg border border-border dark:border-slate-700 shadow-lg h-full flex flex-col transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
        <Quote className="w-10 h-10 text-accent mb-4" />
        <blockquote className="text-text-secondary dark:text-slate-400 italic mb-6 flex-grow">
            “{testimonial.quote}”
        </blockquote>
        <div className="flex items-center mt-auto">
            <img src={testimonial.avatarUrl} alt={testimonial.name} className="w-14 h-14 rounded-full me-4 border-2 border-accent" />
            <div>
                <p className="font-bold font-heading text-text-DEFAULT dark:text-slate-200">{testimonial.name}</p>
                <p className="text-sm text-text-secondary dark:text-slate-400">{testimonial.company}</p>
            </div>
        </div>
    </div>
);
        ...
        <div className="group [perspective:1000px] h-full">
            <div className="relative bg-white dark:bg-slate-800 rounded-lg border border-border dark:border-slate-700 overflow-hidden flex flex-col h-full transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(5deg)] shadow-md hover:shadow-2xl">
                ...
                <div className="p-6 flex flex-col flex-grow">
                    ...
                    <h3 className="text-lg font-bold font-heading text-text-DEFAULT dark:text-slate-200 mb-2 flex-grow">{post.title}</h3>
...
const HomePage: React.FC = () => {
    ...
    return (
        <div className="bg-background dark:bg-slate-900 overflow-x-hidden">
            {/* Hero Section */}
            ...
            <section className="py-24 bg-background-light dark:bg-slate-900">
                 <div className="container mx-auto px-6">
                    <div className="text-center max-w-3xl mx-auto mb-16">
                        <h2 ref={animatedRef} className="timeline-item text-4xl font-extrabold font-heading tracking-tight text-text-DEFAULT dark:text-white mb-4">{t('home.advantage.title')}</h2>
                        <p ref={animatedRef} className="timeline-item text-text-secondary dark:text-slate-400" style={{transitionDelay: '150ms'}}>{t('home.advantage.subtitle')}</p>
                    </div>
            ...
            {/* Featured Products */}
            <section className="py-24 bg-background dark:bg-slate-800">
                <div className="container mx-auto px-6">
                    <div className="text-center max-w-3xl mx-auto">
                        <h2 ref={animatedRef} className="timeline-item text-4xl font-extrabold font-heading tracking-tight text-text-DEFAULT dark:text-white mb-4">{t('home.featuredProducts.title')}</h2>
                        <p ref={animatedRef} className="timeline-item text-text-secondary dark:text-slate-400 mb-16" style={{transitionDelay: '150ms'}}>{t('home.featuredProducts.subtitle')}</p>
                    </div>
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                       {loading ? (
                         Array.from({ length: 3 }).map((_, index) => (
                           <div key={index} className="bg-white dark:bg-slate-700 rounded-lg border border-border dark:border-slate-600 animate-pulse h-[450px]"></div>
                         ))
                       ) : (
            ...
            <section className="py-24 bg-background-light dark:bg-slate-900">
                <div className="container mx-auto px-6">
                    <h2 ref={animatedRef} className="timeline-item text-4xl font-extrabold font-heading tracking-tight text-center text-text-DEFAULT dark:text-white mb-16">{t('home.industries.title')}</h2>
            ...
            {/* CEO Message */}
             <section className="bg-background dark:bg-slate-800">
                <div className="container mx-auto px-6 py-24 sm:py-32 text-center">
                    <div ref={animatedRef} className="timeline-item">
                        <img src="https://images.unsplash.com/photo-1560250097-0b93528c311a?ixlib=rb-4.0.3&q=85&fm=jpg&crop=entropy&cs=tinysrgb&w=120&h=120&fit=crop" alt="Muhammed Rashik P, CEO" className="w-28 h-28 rounded-full mx-auto mb-8 border-4 border-white dark:border-slate-700 shadow-lg"/>
                        <blockquote className="text-2xl sm:text-3xl font-medium font-heading max-w-4xl mx-auto mb-6 leading-snug text-text-DEFAULT dark:text-white">
                            {t('home.ceo.quote')}
                        </blockquote>
                        <cite className="not-italic font-semibold text-lg text-text-secondary dark:text-slate-400">{t('home.ceo.name')}</cite>
                    </div>
                </div>
            </section>
            ...
            <section className="py-24 bg-background-light dark:bg-slate-900">
                <div className="container mx-auto px-6">
                    <h2 ref={animatedRef} className="timeline-item text-4xl font-extrabold font-heading tracking-tight text-center text-text-DEFAULT dark:text-white mb-16">{t('home.testimonials.title')}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                       {loading ? (
                         Array.from({ length: 3 }).map((_, index) => (
                           <div key={index} className="bg-white dark:bg-slate-800 rounded-lg border border-border dark:border-slate-700 animate-pulse h-[350px]"></div>
                         ))
                       ) : (
            ...
            <section className="py-24 bg-background dark:bg-slate-800">
                <div className="container mx-auto px-6">
                    <h2 ref={animatedRef} className="timeline-item text-4xl font-extrabold font-heading tracking-tight text-center text-text-DEFAULT dark:text-white mb-16">{t('home.insights.title')}</h2>
                     <div className="grid grid-cols-1 md:grid-cols-2 max-w-4xl mx-auto gap-8">
                       {loading ? (
                         Array.from({ length: 2 }).map((_, index) => (
                           <div key={index} className="bg-white dark:bg-slate-700 rounded-lg border border-border dark:border-slate-600 animate-pulse h-[350px]"></div>
                         ))
                       ) : (
