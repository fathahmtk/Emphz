        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-start pt-10 pb-10 overflow-y-auto" role="dialog" aria-modal="true">
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-3xl m-4">
                <div className="p-6 border-b dark:border-slate-700 flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-slate-200">{isEditing ? 'Edit Solution' : 'Add New Solution'}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
                    ...
                    <div>
                        <label htmlFor="slug" className="block text-sm font-medium text-gray-700 dark:text-slate-300">Slug (unique identifier)</label>
                        <input type="text" name="slug" id="slug" value={formData.slug} onChange={handleChange} required className={inputClasses}/>
                    </div>
                    <div>
                        <label htmlFor="problem" className="block text-sm font-medium text-gray-700 dark:text-slate-300">The Problem/Challenge</label>
                        <textarea name="problem" id="problem" rows={3} value={formData.problem} onChange={handleChange} required className={inputClasses}></textarea>
                    </div>
                    <div>
                        <label htmlFor="approach" className="block text-sm font-medium text-gray-700 dark:text-slate-300">Our Approach</label>
                        <textarea name="approach" id="approach" rows={4} value={formData.approach} onChange={handleChange} required className={inputClasses}></textarea>
                    </div>
                    <div>
                        <label htmlFor="outcomes" className="block text-sm font-medium text-gray-700 dark:text-slate-300">Outcomes (comma-separated)</label>
                        <input type="text" name="outcomes" id="outcomes" value={formData.outcomes} onChange={handleChange} className={inputClasses}/>
                    </div>
                    <div>
                        <label htmlFor="products" className="block text-sm font-medium text-gray-700 dark:text-slate-300">Product Slugs (comma-separated)</label>
                        <input type="text" name="products" id="products" value={formData.products} onChange={handleChange} className={inputClasses}/>
                    </div>
                    <div>
                        <label htmlFor="caseStudies" className="block text-sm font-medium text-gray-700 dark:text-slate-300">Case Study Titles (comma-separated)</label>
                        <input type="text" name="caseStudies" id="caseStudies" value={formData.caseStudies} onChange={handleChange} className={inputClasses}/>
                    </div>
