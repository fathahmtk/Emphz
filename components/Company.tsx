...
const Company = () => {
    return (
        <section id="company" className="border-t border-zinc-800" role="region" aria-label="Company">
            ...
            <div className="grid grid-cols-1 gap-10 md:grid-cols-2">
                ...
                <p className="mt-2 text-zinc-300">{COMPANY_DATA.standards.description}</p>
                <div className="mt-4 grid grid-cols-2 gap-4 text-sm text-zinc-300">
                    {COMPANY_DATA.standards.tests.map(test => (
                        <div key={test} className="rounded-lg border border-zinc-800 p-3">{test}</div>
                    ))}
                </div>
                </div>
            </div>
