...
const CookieConsentBanner: React.FC = () => {
    ...
    return (
        <div 
            className="fixed bottom-0 start-0 end-0 z-50 bg-primary/[.95] backdrop-blur-sm text-white p-5 shadow-2xl transition-transform duration-500 ease-out"
        ...
        >
            <div className="container mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-start gap-4">
                    <Cookie size={32} className="text-accent flex-shrink-0 mt-1"/>
                    <div>
                        <h2 id="cookie-consent-title" className="font-bold font-heading text-lg">{t('cookies.title')}</h2>
                        <p className="text-sm text-steel-DEFAULT max-w-2xl">{t('cookies.bannerText')}</p>
                    </div>
                </div>
                <div className="flex-shrink-0 flex gap-3 w-full sm:w-auto">
                    ...
                    <button onClick={handleAccept} className="w-1/2 sm:w-auto px-6 py-2 rounded-lg font-semibold bg-accent hover:opacity-90 transition-colors text-sm">
                        {t('cookies.accept')}
                    </button>
                </div>
            </div>
